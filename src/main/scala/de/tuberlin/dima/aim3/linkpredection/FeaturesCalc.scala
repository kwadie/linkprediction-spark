...
object FeaturesCalc {
  ...
  def main(args: Array[String]) {
    ...
    val inputPath = "D:\\datasets\\Small Graph\\edges.txt" //"D:\\datasets\\slashdot0902\\Slashdot0902.txt" 
    ...
    var solSet:RDD[FeatureList] = graph.edges//.sample(false, 1)
     ...
     */
    solSet = solSet.flatMap { f => Seq(  (f.v,f) , (f.u,f) ) }.join(edgesRDD).map( x=>  (x._2._1, x._2._2)   ).groupByKey().mapValues (_.toSet.size).map(f=> f._1.setTotalFriendsOut(f._2)  )
     ...
     */
    solSet  = solSet.flatMap { f => Seq(  (f.v,f) , (f.u,f) ) }.join(edgesRDD).map( f=>  (f._2,1) ).reduceByKey((x,y)=> x+y).map(x=> (x._1._1, x._2 )).groupByKey().mapValues(x =>  x.filter ( _ > 1 ).size ).map( f=> f._1.setCommonFriendsOut(f._2))
